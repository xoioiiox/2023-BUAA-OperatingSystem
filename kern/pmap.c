...
void page_init(void) {
	...
	struct Page *page;
	for (page = pages; page2kva(page) < freemem ; page++) { // 比较条件
		page -> pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for (page = &pages[PPN(PADDR(freemem))]; page < pages + npage ; page++) {
		page -> pp_ref = 0;
...
int page_alloc(struct Page **new) {
	...
	memset(page2kva(pp), 0, sizeof(struct Page));
	*new = pp;
	return 0;
}
