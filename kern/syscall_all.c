...
int sys_set_tlb_mod_entry(u_int envid, u_int func) {
	...
	try(envid2env(envid, &env, 1)); //0 or 1
...
int sys_set_env_status(u_int envid, u_int status) {
	...
	else if ((status == ENV_RUNNABLE) && (env->env_status == ENV_NOT_RUNNABLE)) {
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if ((pa >= 0x13000000) && ((pa + len) <= 0x13004200)) {
		flag = 1;
	}
	if ((pa >= 0x15000000) && ((pa + len) <= 0x15000200)) {
		flag = 1;
	}
	if (flag == 0) {
		return -E_INVAL;
	}
	memcpy(pa + 0xa0000000, va, len);
	return 0;
}
